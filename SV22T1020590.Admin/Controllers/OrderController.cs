...
using ProductModel = SV22T1020590.Models.Catalog.Product;
using OrderModel = SV22T1020590.Models.Sales.Order;
...
namespace SV22T1020590.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        // GET: Order/Search
        [HttpGet]
        public IActionResult Search(string? status = "", string? dateFrom = "", string? dateTo = "", string? searchValue = "", int page = 1, int pageSize = 0)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dateFrom) &&
                DateTime.TryParseExact(dateFrom.Trim(), "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out var fromDateValue))
            {
                fromTime = fromDateValue.Date;
            }
            if (!string.IsNullOrWhiteSpace(dateTo) &&
                DateTime.TryParseExact(dateTo.Trim(), "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out var toDateValue))
            {
                toTime = toDateValue.Date.AddDays(1);
            }

            int rowCount = 0;
            var items = OrderDAL.List(
                _configuration,
                out rowCount,
                searchValue ?? "",
                status ?? "",
            ...
            ViewBag.TotalPages = rowCount > 0 ? (int)Math.Ceiling((double)rowCount / pageSize) : 1;
        ...
        public IActionResult EditCartItem(int id, int productId)
        {
            ...
            if (id > 0)
            {
                ...
                var item = order.OrderDetails.FirstOrDefault(i => i.ProductID == productId)
                           ?? new OrderDetail { ProductID = productId, Quantity = 1, SalePrice = 0m };
        ...
        public IActionResult AddCartItem(int id, int productId, int quantity, decimal salePrice = 0m)
        {
...

            {
                ...
                if (existing == null)
                {
                    ...
                    cart.Add(new OrderDetail { ProductID = productId, Quantity = quantity, SalePrice = price });
                }
                else
                {
        ...
        public IActionResult Shipping(int id)
        {
            ...
            int rowCount;
            var shippers = ShipperDAL.List(_configuration, out rowCount, "", 1, 100);
            ViewBag.Shippers = shippers
                .Select(s => new SelectListItem
                {
                    Value = s.ShipperID.ToString(),
                    Text = $"{s.ShipperName} - {s.Phone}",
                    Selected = order.ShipperID == s.ShipperID
                })
                .ToList();
            return View();
        }
        ...
        public IActionResult ShippingPost(int id, int shipperID)
        {
            ...
            OrderDAL.SetStatus(_configuration, id, ((int)OrderStatusEnum.Shipping).ToString(), CurrentEmployeeId(), shipperID);
            TempData["Success"] = $"Order {id} marked shipping.";
            return RedirectToAction(nameof(Detail), new { id });
        }

        // GET: Order/Finish/5 (modal)
        [HttpGet]
